...
class PayrollController extends Controller
{
    public function index()
    {
        ...
        $payslip = Payroll::join('users', 'users.id', 'payrolls.user_id')
            ->join('personal_tables', 'personal_tables.user_id', 'users.id')
            ->where('users.id', Auth::user()->id)
            ->get()
            ->first();
        $lop_leaves = Leave::where('leaves.user_id',Auth::user()->id)->where('leave_type','Loss of Pay')->where('leave_type','Loss of Pay (Half Day)')->count();
        return view('pages.payroll', ['myreq' => $myreq, 'payslip' => $payslip, 'lop_leaves' => $lop_leaves]);
    }
    ...
    public function generatePDF()
    {
        ...
        $pdf = PDF::loadView('pages.payslip_pdf', $data);
     
        return $pdf->download(Auth::user()->name.'Payslip.pdf');
    }
    ...
    public function edit($id)
    {
        ...
        $employee = personal_table::join('users', 'users.id', 'personal_tables.user_id')
            ...
            ->select('payrolls.*', 'personal_tables.*', 'users.empcode', 'users.ctc')
            ->get()
            ->first();
    ...
    public function update(Request $request, $id)
    {
        ...
        $personal_table->others = $request->others;
        $personal_table->save();
    ...
    {
        $preq = Requests::join('users', 'users.id', 'requests.user_id')
